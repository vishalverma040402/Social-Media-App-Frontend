          ...
          <div className="user-name-userid">
            <img src={profileImg ? (profileImg) : Profile} alt="" />
              <div className='L-user'>
                <h1>{modelDetails ? (modelDetails.ModelName) : "Vishal"}</h1>
                <span>{modelDetails ? (modelDetails.ModelUserName) : "@Vishal"}</span>
            </div>
          </div>
        </Link>
        <MoreHorizIcon onClick={()=>setLogOutExit(!logOutExit)} className='vert'/>
          ...
          {logOutExit && (
            <div className="logOutExitContainer">
              ...
              <Link to="/" style={{width:"100%"}}><button>Log out @Vishal</button></Link>
            </div>
          )}
      </div>
